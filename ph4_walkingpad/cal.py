...
class CalMeter:
    ...
    def work(self):
        ...
        timx = self.args.time  # type: Union[str,int,float]
        dist = self.args.dist
        ...
        if timx:
            ...
            if isinstance(timx, str):
                ...
                logger.debug('Time %s recomputed to %s seconds' % (old_time, timx,))
        ...
        ccal_net = ccal - (timx / 60) * calories_rmrcb_minute(self.profile.weight, self.profile.height,
                                                              self.profile.age, self.profile.male)

        print("Speed: %4.1f km/h, dist: %5.2f km, time: %5s s = %02s :%02s :%02s, cal: %7.2f, ncal: %7.2f"
    ...
    def load_profile(self):
        ...
        if self.args.profile:
            with open(self.args.profile, 'r') as fh:
                dt = json.load(fh)
                self.profile = Profile(pid=dt['id'], male=dt['male'], age=dt['age'], weight=dt['weight'],
                                       height=dt['height'])
    ...
    def argparser(self):
        ...
        parser.add_argument('-w', '--weight', dest='weight', type=float, default=None,
                            help='Weight of the person in kg')
        parser.add_argument('-H', '--height', dest='height', type=float, default=None,
                            help='Height of the person in m')
        parser.add_argument('-a', '--age', dest='age', type=float, default=None,
                            help='Age of the person')
        parser.add_argument('-f', '--female', dest='female', action='store_const', const=True,
                            help='Person is a female')
        parser.add_argument('-s', '--speed', dest='speed', type=float,
                            help='Speed of the walk in km/h')
        parser.add_argument('-t', '--time', dest='time',
                            help='Time of the walk in hours h:m:s or 1h 2m 3s')
        parser.add_argument('-d', '--dist', dest='dist',
                            help='Distance of the walk in km')
        return parser
