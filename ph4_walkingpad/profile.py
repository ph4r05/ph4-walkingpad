...
def calories_walk2_minute(speed: float, weight: float, deg: float):
    ...
    if deg <= 0.06:
        ...
        return 1/60. * sum([speed_matrix[matrix_idx][i] * (speed ** (3-i)) for i in range(4)]) * weight

    mpm = speed * 1000 / 60
    return 1/60. * ((0.1 * mpm + 1.8 * mpm * deg + 3.5) * weight * 60 * 5 / 1000)


class Profile:
    def __init__(self, pid=None, male=True, age=0, weight=0, height=0, token=None, did=None):
        self.pid = pid
        self.male = male
        self.age = age
        self.weight = weight
    ...
    def __str__(self):
        return 'Profile(pid=%s, male=%s, age=%s, weight=%s, height=%s)' \
               % (self.pid, self.male, self.age, self.weight, self.height)
